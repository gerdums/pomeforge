...
Release bundles include Ubuntu Jammy ncurses 6.3-2ubuntu0.3 narrow `libncurses` and `libtinfo` compatibility libraries under the [upstream copyright and permissive license](docs/third-party/ncurses-COPYRIGHT.txt). The original package and payload SHA-256 pins are recorded in `packaging/release/runtime-lock.json`; package provenance records the private copies. Only the private ncurses copy receives a relative library search path so it can find its matching tinfo library. Swift's executable bytes remain unchanged.
