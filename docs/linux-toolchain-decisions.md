...
unxip 3.3 requires its output directory to exist. Pomeforge must create an empty, private extraction directory before launching it; passing a nonexistent child path fails before extraction. Keep the SDK builder's destination separate from that extraction directory.
...
The SDK version and system product version are different facts. Do not substitute one for the other or invent missing DT metadata. Retaining Xcode archive provenance also does not mean Xcode executed the build. No Apple SDK is redistributed with Pomeforge.
