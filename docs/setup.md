...
```sh
pomeforge tools install asc --json
pomeforge tools install asc --execute --json
...
and download size. Execution delegates to Pomeforge's reviewed bootstrap
installer, verifies the complete installed tree and active link, and never uses
`sudo`, changes the global `PATH`, or runs a remote shell script. `pomeforge
doctor` and `pomeforge tools` only perform bounded read-only discovery; they do
...
Build Pomeforge's pinned Linux AssetKit bridge from this checkout:

```sh
swift build --package-path tools/asset-compiler -c release
pomeforge_assets=$(realpath "$PWD/tools/asset-compiler/.build/release/pomeforge-assets")
pomeforge tools register pomeforge-assets \
  --path "$pomeforge_assets" \
...
Pomeforge runs `--help` because the bridge intentionally has no `--version`.
Source and AssetKit revisions are retained only when the operator supplies
them; Pomeforge does not invent provenance from a path or binary.
...
```sh
pomeforge tools register unxip --path /absolute/path/unxip \
  --source-revision 6c3990517fcc4c1db6952fccf4c562fb14097601 \
  --execute
```

Packaged `/usr/local/bin/pomeforge-assets` and `/usr/local/bin/unxip` files are
...
Pomeforge never downloads Xcode or Apple SDK content. Supply the downloaded XIP
or an already extracted `Xcode.app` directory:

```sh
pomeforge sdk status --execute --json
pomeforge sdk import --input "$HOME/Downloads/Xcode.xip" --arch x86_64 --json
pomeforge sdk import --input "$HOME/Downloads/Xcode.xip" --arch x86_64 --execute --json
```
...
For XIP input, Pomeforge runs `unxip --statistics INPUT.xip FRESH_OUTPUT_DIR`,
...
Pomeforge copies OUTPUT_PARENT/darwin.xtoolsdk into a fresh user-owned darwin.artifactbundle
Pomeforge copies CLANG_RESOURCE_DIR/include into that staged artifact bundle
...
retained for diagnosis and an existing Darwin SDK is never replaced: Pomeforge
...
configuration root are fingerprinted before execution. Pomeforge never invokes
the normal-user-incompatible `xtool sdk install` copy path and never uses
`sudo`. Exit zero is insufficient when status says `Not installed`. Pomeforge
...
tool discovery, status, import, and project builds. Pomeforge honors an absolute
