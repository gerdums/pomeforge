...
asset_stage=$(awk '/AS orchard-assets-build/{copy=1} /AS orchard-unxip-build/{copy=0} copy' "$containerfile")
asset_out_line=$(printf '%s\n' "$asset_stage" | grep -nF -- 'install -d /out' | cut -d: -f1)
asset_install_line=$(printf '%s\n' "$asset_stage" | grep -nF -- 'install -m 0755 tools/asset-compiler/.build/release/orchard-assets' | cut -d: -f1)
test "$asset_out_line" -lt "$asset_install_line"
assert_contains 'AS orchard-unxip-build' "$containerfile"
...
assert_contains 'COPY --from=orchard-unxip-build /out/unxip /usr/local/bin/unxip' "$containerfile"
assert_contains 'COPY --from=orchard-unxip-build /out/unxip-3.3.0.tar.gz /usr/share/orchard/sources/unxip-3.3.0.tar.gz' "$containerfile"
assert_contains 'COPY --from=orchard-unxip-build /out/unxip-3.3.0.LICENSE /usr/share/orchard/sources/unxip-3.3.0.LICENSE' "$containerfile"
assert_contains 'COPY THIRD_PARTY_NOTICES.md /usr/share/doc/orchard/THIRD_PARTY_NOTICES.md' "$containerfile"
assert_contains 'COPY docs/third-party/ /usr/share/doc/orchard/third-party/' "$containerfile"
...
assert_contains 'run: CGO_ENABLED=0 GOOS=linux go build -trimpath -o /tmp/orchard ./cmd/orchard' "$workflow"
