...
func TestManifestRejectsUnknownAndUnsupportedValues(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "pomeforge.json"), []byte(valid), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(project, "pomeforge.json"), []byte(unknown), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(project, "pomeforge.json"), []byte(unsupported), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestManifestAcceptsResourceIDsAndRejectsControlLikeIDs(t *testing.T) {
	manifest := Manifest{
		...
		AppStore: AppStoreIDs{AppID: "123456789", VersionID: "a1b2c3d4-1111-4222-8333-abcdef123456", BuildID: "b2c3d4e5-2222-4333-8444-bcdefa234567"},
	}
...
func TestLoadManifestRejectsSymlinkWithoutChangingTarget(t *testing.T) {
	...
	if err := os.Symlink(target, filepath.Join(project, "pomeforge.json")); err != nil {
		t.Fatal(err)
	}
