...
func (m *SetupManager) planSDKImport(ctx context.Context, input PlanInput, hostArch string, plan *Plan) (string, error) {
	...
	if xtool.Status == "available" {
		...
		if statusErr != nil || statusCode != 0 {
			...
			plan.Blockers = append(plan.Blockers, "a Darwin SDK is already installed; Pomeforge refuses to replace it")
		}
	}
	...
	plan.Steps = append(plan.Steps,
		...
		Step{Kind: "internal", Tool: "pomeforge", Operation: "stage-sdk-artifactbundle", Parameters: map[string]string{"source": filepath.Join(buildParent, "darwin.xtoolsdk"), "destination": artifactBundle, "clangHeaders": filepath.Join(clangResourceDir, "include"), "clangHeadersSha256": clangHeadersDigest}, Description: "Copy the built SDK into a fresh user-owned artifact bundle and replace its Clang headers from the selected host Clang resource tree."},
		Step{Kind: "internal", Tool: "pomeforge", Operation: "refuse-sdk-replacement", Parameters: map[string]string{"statusCommand": "xtool sdk status"}, Description: "Recheck actual SDK status immediately before installation and refuse replacement if a Darwin SDK is installed."},
	...
	plan.Warnings = append(plan.Warnings,
		"The Apple input is operator supplied and is never downloaded by Pomeforge.",
...
func (m *SetupManager) executeSDKImport(ctx context.Context, plan Plan, input PlanInput) (string, int, map[string]string, error) {
	...
	_, _ = output.Write([]byte("[pomeforge] retained and hash-verified named SDK metadata snapshot\n"))
	...
	_, _ = output.Write([]byte("[pomeforge] staged fresh user-owned darwin.artifactbundle with selected Clang headers\n"))
