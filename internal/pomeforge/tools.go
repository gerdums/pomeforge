...
func (r SystemToolResolver) probeDefinition(ctx context.Context, definition toolDefinition) ToolStatus {
	...
	if definition.id == "xtool" && !regexp.MustCompile(`(?i)^xtool\s+1\.19\.0(?:\s|$)`).MatchString(version) {
		status.Status = "unverified"
		status.Detail = "Pomeforge has verified only xtool 1.19.0 command contracts; this version is not verified"
	}
	if definition.id == "asc" && !regexp.MustCompile(`(?i)^(?:asc\s+version\s+)?5\.[0-9]+(?:\.[0-9]+)?(?:\s|$)`).MatchString(version) {
		status.Status = "incompatible"
		status.Detail = "Pomeforge has verified ASC 5.x command contracts; found a different version"
	}
	if definition.id == "swift" {
		matches := regexp.MustCompile(`(?i)Swift version\s+([0-9]+)\.([0-9]+)`).FindStringSubmatch(version)
		if len(matches) != 3 {
			status.Status = "unverified"
			status.Detail = "Swift ran, but Pomeforge could not parse its version"
		} else {
