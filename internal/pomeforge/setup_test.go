...
func TestWorkspaceHistoryFailureReturnsPopulatedResult(t *testing.T) {
	...
	service.Setup.Runner = setupRunnerFunc(func(_ context.Context, _ string, _ []string, _ string, _ []string) (string, int, error) {
		historyPath := filepath.Join(historyRoot, ".pomeforge", "history.jsonl")
...
func TestBlockedCapabilitiesDescribeActualMissingPrerequisites(t *testing.T) {
	...
	tools := fakeTools{
		...
		"pomeforge-assets": {ID: "pomeforge-assets", Name: "Pomeforge AssetKit bridge", Status: "unverified", Detail: "receipt mismatch"},
...
func TestHelperRegistrationStoresIntegrityAndKnownAssetKitPin(t *testing.T) {
	...
	helper := filepath.Join(root, "pomeforge-assets")
	...
	input := PlanInput{Action: "helper-register", Helper: "pomeforge-assets", ExecutablePath: helper, AssetKitRevision: assetKitRevision}
	plan, err := service.PlanOperation(context.Background(), input, true)
	if err != nil || !plan.Executable {
		t.Fatalf("plan=%#v err=%v", plan, err)
	}
	...
	if status := resolver.Probe(context.Background(), "pomeforge-assets"); status.Status != "available" {
