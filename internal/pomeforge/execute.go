...
func (e *Executor) Execute(ctx context.Context, plan Plan, project string, confirm bool) (OperationResult, error) {
	...
	if actionRequiresLinux(plan.Action) && hostOS != "linux" {
		return OperationResult{}, Errorf("blocked", "this compile, signing, or export workflow is supported only when Pomeforge is running on Linux")
	}
	if !plan.Executable {
...
func scanHistory(reader io.Reader, limit int) ([]OperationResult, error) {
	...
	for scanner.Scan() {
		...
		if json.Unmarshal(scanner.Bytes(), &result) == nil {
			// History may have been written by an older Pomeforge redactor. Apply
...
func openHistoryWithin(workspace, project string, flags int, create bool) (*os.File, error) {
	...
	if create {
		if err := syscall.Mkdirat(projectFD, ".pomeforge", 0o700); err != nil && !errors.Is(err, syscall.EEXIST) {
			return nil, err
		}
	}
	directoryFD, err := syscall.Openat(projectFD, ".pomeforge", syscall.O_RDONLY|syscall.O_DIRECTORY|syscall.O_CLOEXEC|syscall.O_NOFOLLOW, 0)
	if err != nil {
		return nil, fmt.Errorf(".pomeforge must be a private directory and not a symlink: %w", err)
	}
	...
	return os.NewFile(uintptr(fileFD), filepath.Join(project, ".pomeforge", "history.jsonl")), nil
}
