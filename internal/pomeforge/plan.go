...
var actionCatalog = []ActionInfo{
	...
	{ID: "helper-register", Title: "Register helper", Description: "Validate and register an existing pomeforge-assets or unxip executable with an integrity digest.", Effect: "local-write", Scope: "workspace", Parameters: []ActionParameter{{Name: "helper", Type: "enum", Required: true, Description: "Helper executable contract.", Values: []string{"pomeforge-assets", "unxip"}}, {Name: "executablePath", Type: "absolute-path", Required: true, Description: "Existing executable to hash and probe."}, {Name: "sourceRevision", Type: "string", Description: "Known source revision; never inferred."}, {Name: "assetKitRevision", Type: "string", Description: "Known AssetKit revision for pomeforge-assets; never inferred."}}},
	...
	{ID: "signing-configure", Title: "Configure signing identity", Description: "Create a named private signing configuration after inspecting its certificate and profile.", Effect: "local-write", Scope: "workspace", Parameters: []ActionParameter{{Name: "identity", Type: "identity-id", Required: true, Description: "New safe named identity ID."}, {Name: "identityLabel", Type: "string", Required: true, Description: "Public display label."}, {Name: "privateKeyPath", Type: "absolute-private-path", Required: true, Description: "Existing unencrypted private key outside the workspace; never returned."}, {Name: "certificatePath", Type: "absolute-private-path", Required: true, Description: "Existing PEM certificate outside the workspace; never returned."}, {Name: "profilePath", Type: "absolute-private-path", Required: true, Description: "Existing provisioning profile outside the workspace; never returned."}, {Name: "trustedRootPaths", Type: "absolute-private-path-list", Description: "Optional explicit operator trust roots outside the workspace; never returned."}}},
	{ID: "signing-inspect", Title: "Inspect signing identity", Description: "Inspect public certificate, profile, entitlement, and explicit trust-root facts for a named identity.", Effect: "local-read", Scope: "workspace", Parameters: []ActionParameter{{Name: "identity", Type: "identity-id", Required: true, Description: "Configured named identity ID."}}},
	...
	{ID: "install", Title: "Install on device", Description: "Provision/development-sign and install an IPA, or build, provision, and install a debug app on a selected device. Launch remains separate.", Effect: "device-write", RequiresConfirmation: true, Scope: "project"},
	...
	{ID: "release-build", Title: "Build unsigned release bundle", Description: "Build an unsigned release .app with the validated active iOS SDK and truthful release metadata.", Effect: "local-build", Scope: "project", Parameters: []ActionParameter{{Name: "sourceBundle", Type: "project-path", Description: "Expected unsigned .app output to inspect after building."}}},
	{ID: "icons", Title: "Customize starter icon", Description: "Replace Pomeforge starter art with a complete deterministic iPhone, iPad, and marketing icon catalog from a 1024px PNG.", Effect: "local-write", Scope: "project", Parameters: []ActionParameter{{Name: "iconSource", Type: "project-png-path", Required: true, Description: "Existing 1024x1024 PNG inside the project."}}},
	{ID: "export", Title: "Export signed IPA", Description: "Create and inspect an App Store distribution-signed IPA through the native Linux distribution pipeline.", Effect: "local-build", Scope: "project", Parameters: []ActionParameter{{Name: "sourceBundle", Type: "project-app-path", Required: true, Description: "Existing unsigned release .app."}, {Name: "identity", Type: "identity-id", Required: true, Description: "Configured named signing identity."}, {Name: "outputIPA", Type: "new-project-ipa-path", Required: true, Description: "New no-overwrite IPA destination."}, {Name: "assetCatalog", Type: "project-catalog-path", Description: "Optional catalog compiled before signing."}}},
	{ID: "ipa-inspect", Title: "Inspect IPA", Description: "Inspect IPA structure, Mach-O, SDK metadata, embedded profile, icons, and SHA-256 without uploading.", Effect: "local-read", Scope: "project", Parameters: []ActionParameter{{Name: "ipa", Type: "project-ipa-path", Required: true, Description: "Existing IPA to inspect."}, {Name: "identity", Type: "identity-id", Description: "Optional identity whose explicit trust roots are used."}}},
	{ID: "store-status", Title: "Inspect store build", Description: "Read App Store Connect build status for the configured build ID.", Effect: "account-read", Scope: "project", Parameters: []ActionParameter{{Name: "buildId", Type: "string", Required: true, Description: "App Store Connect build resource ID or manifest fallback."}}},
	{ID: "validate", Title: "Validate store readiness", Description: "Run ASC's remote App Store version readiness validation.", Effect: "account-read", Scope: "project", Parameters: []ActionParameter{{Name: "appId", Type: "numeric-id", Required: true, Description: "App Store app ID or manifest fallback."}, {Name: "versionId", Type: "string", Description: "Version resource ID; mutually exclusive with version."}, {Name: "version", Type: "version", Description: "Marketing version; mutually exclusive with versionId."}}},
	{ID: "upload", Title: "Upload IPA", Description: "Upload a pre-exported, correctly distribution-signed IPA to App Store Connect.", Effect: "account-write", RequiresConfirmation: true, Scope: "project", Parameters: []ActionParameter{{Name: "ipa", Type: "project-ipa-path", Required: true, Description: "Exact IPA to inspect, bind, and upload."}, {Name: "identity", Type: "identity-id", Required: true, Description: "Identity whose profile and roots must match the IPA."}, {Name: "appId", Type: "numeric-id", Required: true, Description: "App Store app ID or manifest fallback."}}},
	{ID: "submit", Title: "Submit for review", Description: "Dry-run and then submit a configured build for App Review.", Effect: "account-write", RequiresConfirmation: true, Scope: "project", Parameters: []ActionParameter{{Name: "appId", Type: "numeric-id", Required: true, Description: "App Store app ID or manifest fallback."}, {Name: "versionId", Type: "string", Required: true, Description: "App Store version resource ID or manifest fallback."}, {Name: "buildId", Type: "string", Required: true, Description: "App Store build resource ID or manifest fallback."}}},
}
...
func (p Planner) Plan(ctx context.Context, input PlanInput) (Plan, error) {
	...
	if isReleaseAction(input.Action) {
		return Plan{}, Errorf("release_service_required", "release actions must be planned through Pomeforge's shared release service")
	}
	...
	if actionRequiresLinux(input.Action) && hostOS != "linux" {
		plan.Blockers = append(plan.Blockers, "this compile, signing, or export workflow is supported only when Pomeforge is running on Linux")
	}
	...
	case "setup":
		...
		plan.Warnings = append(plan.Warnings, "SDK import requires an operator-supplied Xcode.app or XIP through `pomeforge sdk import`; Pomeforge does not download Apple SDKs or replace an installed SDK.")
	case "build":
	...
	case "install":
		...
		if input.IPA != "" {
			...
			addToolStep("xtool", "Build, provision, and install a debug app on the selected device.", "dev", "run", "--configuration", "debug", "--udid", input.Device)
		}
		plan.Warnings = append(plan.Warnings, "xtool 1.19 dev run builds, provisions, and installs but does not launch. Use Pomeforge's separate launch action afterward. xtool install/dev run may development-sign the app and change its signing identity; it does not preserve or produce an App Store distribution signature.")
...
func (p Planner) fingerprint(ctx context.Context, project string, input PlanInput, tools []string) (string, error) {
	h := sha256.New()
	writeFingerprintRecord(h, "schema", []byte("pomeforge-plan-v3"))
	...
	err := walkRegularFilesInProject(ctx, p.Workspace, project, map[string]bool{".git": true, ".pomeforge": true, ".build": true, "xtool": true}, func(relative string, file *os.File) error {
