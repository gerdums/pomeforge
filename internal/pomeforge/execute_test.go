...
func TestProcessHelper(t *testing.T) {
	...
	case "break-history":
		history := filepath.Join(os.Args[separator+2], ".pomeforge", "history.jsonl")
...
func TestExecutorFailureOutputBoundsRedactionAndHistory(t *testing.T) {
	...
	info, err := os.Stat(filepath.Join(project, ".pomeforge", "history.jsonl"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestExecutorEnforcesLinuxForBuildActions(t *testing.T) {
	project := t.TempDir()
	if _, err := (&Executor{HostOS: "darwin"}).Execute(context.Background(), helperPlan(project, "large"), project, false); err == nil || !strings.Contains(err.Error(), "only when Pomeforge is running on Linux") {
		t.Fatalf("non-Linux executor accepted build plan: %v", err)
	}
}

func TestChildEnvironmentIsNarrow(t *testing.T) {
	t.Setenv("POMEFORGE_TEST_SECRET", "must-not-pass")
	...
	if strings.Contains(buildEnvironment, "POMEFORGE_TEST_SECRET") || strings.Contains(buildEnvironment, "ASC_CONFIG_PATH") {
...
func TestHistoryAppendRejectsProjectAncestorReplacedByCompletedOperation(t *testing.T) {
	...
	outsideHistoryDirectory := filepath.Join(outsideProject, ".pomeforge")
...
func TestHistoryRejectsSymlink(t *testing.T) {
	project := t.TempDir()
	if err := os.Mkdir(filepath.Join(project, ".pomeforge"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Symlink(outside, filepath.Join(project, ".pomeforge", "history.jsonl")); err != nil {
		t.Fatal(err)
	}
...
func TestHistoryRejectsSpecialAndOversizedFiles(t *testing.T) {
	project := t.TempDir()
	directory := filepath.Join(project, ".pomeforge")
