...
func (r *IntegratedToolResolver) ProbeAll(ctx context.Context) []ToolStatus {
	...
	ids = append(ids, "orchard-assets", "unxip")
...
func (r *IntegratedToolResolver) probeHelper(ctx context.Context, id string) ToolStatus {
	name := map[string]string{"orchard-assets": "Orchard AssetKit bridge", "unxip": "unxip XIP extractor"}[id]
	...
	if info, statErr := os.Lstat(packaged); statErr == nil && info.Mode().IsRegular() {
		if packagedReceipts, receiptErr := loadHelperReceipts("/usr/local/share/orchard/helpers.json"); receiptErr == nil {
...
func verifyHelper(ctx context.Context, receipt HelperReceipt, fallback ToolStatus) ToolStatus {
	...
	if receipt.ID == "unxip" && receipt.SourceRevision != "" && receipt.SourceRevision != unxipRevision {
		status.Status = "unverified"
		status.Detail = "registered unxip source revision does not match Orchard's reviewed revision"
		return status
	}
	if receipt.ID == "orchard-assets" && receipt.AssetKitRevision != "" && receipt.AssetKitRevision != assetKitRevision {
		status.Status = "unverified"
		status.Detail = "registered AssetKit revision does not match Orchard's pinned revision"
		return status
	}
	...
	if receipt.ID == "unxip" && probed.Version != "unxip 3.3" {
		probed.Status = "unverified"
		probed.Detail = "Orchard has verified the unxip 3.3 command contract"
		return probed
	}
	if receipt.ID == "orchard-assets" {
		help := strings.ToLower(probed.Version)
		if !strings.Contains(help, "orchard-assets") || !strings.Contains(help, "compile") {
			probed.Status = "unverified"
			probed.Detail = "registered executable did not report the orchard-assets compile help contract"
			return probed
		}
	}
	if receipt.ID == "unxip" {
...
func helperGuidance(id string) string {
	...
	return "run `swift build --package-path tools/asset-compiler -c release`, then register the resulting orchard-assets executable"
}
...
func loadHelperReceipts(path string) ([]HelperReceipt, error) {
	...
	for _, receipt := range registry.Helpers {
		if receipt.ID != "orchard-assets" && receipt.ID != "unxip" {
