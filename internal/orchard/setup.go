...
import (
	...
	toolcatalog "orchard.local/orchard"
...
func validateWorkspaceInput(input PlanInput) error {
	...
	case "helper-register":
		...
		if input.Helper == "unxip" && input.AssetKitRevision != "" {
			return Errorf("invalid_input", "assetKitRevision applies only to orchard-assets")
		}
	case "sdk-import":
...
func (m *SetupManager) planToolInstall(ctx context.Context, input PlanInput, hostOS, hostArch string, plan *Plan) (string, error) {
	...
	plan.Steps = append(plan.Steps, Step{
		Kind: "internal", Tool: "orchard", Operation: "verified-managed-install",
	...
	plan.Warnings = append(plan.Warnings, "This writes only to Orchard's private XDG data/cache paths and does not use sudo or modify the host PATH.")
	managedState := "absent"
...
func (m *SetupManager) planHelperRegister(ctx context.Context, input PlanInput, plan *Plan) (string, error) {
	if input.Helper != "orchard-assets" && input.Helper != "unxip" {
		return "", Errorf("unsupported_helper", "helper must be orchard-assets or unxip")
	}
	...
	if input.Helper == "orchard-assets" && input.AssetKitRevision != "" && input.AssetKitRevision != assetKitRevision {
		plan.Blockers = append(plan.Blockers, "AssetKit revision does not match Orchard's pinned bridge revision "+assetKitRevision)
	}
	...
	plan.Steps = append(plan.Steps, Step{Kind: "internal", Tool: "orchard", Operation: "register-helper-integrity", Parameters: map[string]string{"helper": input.Helper, "executablePath": path, "sha256": digest, "sourceRevision": input.SourceRevision, "assetKitRevision": input.AssetKitRevision}, Description: "Recheck the executable and command contract, then store its exact path, SHA-256, and supplied provenance in private user state."})
	if input.SourceRevision == "" {
		plan.Warnings = append(plan.Warnings, "No source revision was supplied; Orchard will not infer provenance from the executable path.")
	}
	if input.Helper == "orchard-assets" && input.AssetKitRevision == "" {
		plan.Warnings = append(plan.Warnings, "No AssetKit revision was supplied; Orchard will not claim which AssetKit source produced this executable.")
	}
