...
func loadManifestWithin(workspace, project string) (Manifest, []byte, error) {
	...
	if err := decodeOne(decoder, &manifest); err != nil {
		return Manifest{}, nil, Errorf("invalid_manifest", "decode orchard.json: "+err.Error())
	}
...
func DiscoverProjects(workspace string) ([]ProjectSummary, error) {
	...
	err := filepath.WalkDir(workspace, func(projectPath string, entry fs.DirEntry, walkErr error) error {
		...
		manifestInfo, statErr := os.Lstat(filepath.Join(projectPath, "orchard.json"))
...
const releaseTemplate = `# Release boundary

Orchard's generated project supports Linux development builds through xtool.
The release IPA produced by the initial export action is unsigned. App Store
distribution requires a separately verified distribution identity, profile,
entitlements, signing operation, and archive validation. Development signing is
not App Store signing. Upload and review submission are explicit confirmed steps.
`
